...
def get_file_to_dict_format_txt(directory='test-files'):
    data = {}
    for filename in os.listdir(directory):
        path = "./{0}/{1}".format(directory, filename)
        with open(path, 'r') as file_in:
            file_in_dict_format = {}
            bz_file = file_in.readlines()
            for counter, line in enumerate(bz_file, 1):
                if line != '\n':
                    file_in_dict_format[counter] = line.strip('\n').strip()
        data[filename] = file_in_dict_format
...
def main():
    print("Get files ...")
    files_in_dict_format = get_file_to_dict_format_txt()

    with open('test-dict-format.json', 'w') as json_out:
